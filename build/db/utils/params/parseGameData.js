export const parseGameData = (game, mode) => {
    if (!game.name)
        throw new Error(`Missing required field: name. Recieved: ${game.name}`);
    if (!game.steam_appid)
        throw new Error(`Missing required field: steam_appid. Recieved: ${game.steam_appid}`);
